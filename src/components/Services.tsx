...
const Services = () => {
  ...
  return (
    <section id="services" className="py-24 bg-card relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,hsl(45_85%_55%/0.03)_0%,transparent_50%)]" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {services.map((service, index) => (
            <AnimatedSection 
              ...
              delay={index * 0.05}
            >
              <div className="group relative p-6 rounded-2xl bg-background/50 backdrop-blur-sm border border-border hover:border-primary/40 transition-all duration-500 hover:-translate-y-1 h-full">
                {/* Hover Glow */}
                ...
                <div className="relative">
                  ...
                  {/* Content */}
                  <h3 className="font-display text-lg text-foreground mb-2 text-left">
                    {service.title}
                  </h3>
                  <p className="font-sans text-muted-foreground text-sm leading-relaxed mb-4 text-left">
                    {service.description}
                  </p>
                  ...
                  <ul className="space-y-1.5">
                    {service.features.slice(0, 3).map((feature) => (
                      <li key={feature} className="flex items-center gap-2 font-sans text-xs text-foreground/80 text-left">
