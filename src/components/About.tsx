...
const About = () => {
  ...
  return (
    <section id="about" className="py-24 bg-gradient-dark relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,hsl(45_15%_18%/0.05)_1px,transparent_1px),linear-gradient(to_bottom,hsl(45_15%_18%/0.05)_1px,transparent_1px)] bg-[size:80px_80px]" />
      
      <div className="container mx-auto px-6 relative">
        {/* Overview */}
        <AnimatedSection className="max-w-4xl mb-20" direction="left">
          ...
          <h2 className="font-display text-3xl md:text-4xl lg:text-5xl text-foreground mb-8 leading-tight text-left">
          ...
          <div className="space-y-8">
            ...
            <AnimatedSection direction="left" delay={0.1}>
              <div className="group p-8 rounded-3xl bg-card/50 backdrop-blur-sm border border-border hover:border-primary/30 transition-all duration-500">
                ...
                <p className="font-sans text-muted-foreground leading-relaxed text-left">
            ...
            <AnimatedSection direction="left" delay={0.3}>
              <div>
                <h3 className="font-display text-xl text-foreground mb-5 text-left">Our Core Values</h3>
                <div className="grid grid-cols-2 gap-3">
                  {values.map((value, index) => (
                    ...
                    >
                      ...
                      <p className="font-sans text-sm text-muted-foreground pl-11 text-left">{value.description}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-8">
            <AnimatedSection direction="right" delay={0.2}>
              <div className="p-8 rounded-3xl bg-card/50 backdrop-blur-sm border border-border">
                <h3 className="font-display text-2xl text-foreground mb-4 text-left">
                  We Are a <span className="text-gradient-gold">Professional</span> Team
                </h3>
                <p className="font-sans text-muted-foreground mb-8 leading-relaxed text-left">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {roles.map((role, index) => (
                    ...
                    >
                      ...
                      <span className="font-sans text-sm text-foreground text-left">{role}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={item.label} className="flex justify-between items-center py-3 border-b border-primary/10 last:border-0">
                      <span className="font-sans text-foreground text-left">{item.label}</span>
